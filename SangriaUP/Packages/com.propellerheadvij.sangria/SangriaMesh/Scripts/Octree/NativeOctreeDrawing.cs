	...
	public unsafe partial struct NativeOctree<T> where T : unmanaged
	{
			...
			for (int i = 0; i < tree.nodes->Capacity; i++)
			{
				...
				if(node.count > 0)
				{
					for (int k = 0; k < node.count; k++)
					{
						...
						// Draw 2d to texture
						SetPixelSafe(texture,
							(int) ((element.pos.x + widthAdd) * widthMult),
							(int) ((element.pos.y + heightAdd) * heightMult),
							Color.red);
					}
				}
			}
		...
		static void DrawBounds(Color[][] texture, AABB bounds, NativeOctree<T> tree)
		{
			...
			for (int leftToRight = 0; leftToRight < bounds.Extents.x * 2; leftToRight++)
			{
				...
				SetPixelSafe(texture,
					(int) ((poxX + widthAdd) * widthMult),
					(int) ((bounds.Center.y + heightAdd + bounds.Extents.y) * heightMult),
					Color.blue);
				SetPixelSafe(texture,
					(int) ((poxX + widthAdd) * widthMult),
					(int) ((bounds.Center.y + heightAdd - bounds.Extents.y) * heightMult),
			...
			for (int topToBottom = 0; topToBottom < bounds.Extents.y * 2; topToBottom++)
			{
				...
				SetPixelSafe(texture,
					(int) ((bounds.Center.x + widthAdd + bounds.Extents.x) * widthMult),
					(int) ((posY + heightAdd) * heightMult),
					Color.blue);
				SetPixelSafe(texture,
					(int) ((bounds.Center.x + widthAdd - bounds.Extents.x) * widthMult),
					(int) ((posY + heightAdd) * heightMult),
		...
		static void SetPixelSafe(Color[][] texture, int x, int y, Color color)
		{
			if (x < 0 || x >= texture.Length)
			{
				return;
			}

			if (y < 0 || y >= texture[x].Length)
			{
				return;
			}
